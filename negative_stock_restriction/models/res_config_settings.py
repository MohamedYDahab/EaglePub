...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        ``res.config.settings`` maps ``0`` to ``False`` and
        ``set_param`` then *deletes* the record, so a threshold or interval
        of 0 would silently fall back to its default. Booleans are left
        alone: Odoo represents an unticked box as an absent parameter, and
        ``default_get`` reads any stored string back as ``bool(value)`` --
        so writing the literal ``'False'`` would render the box as ticked.
        """
        super().set_values()
