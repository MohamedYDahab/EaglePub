...
class PosSession(models.Model):
    ...
    def _get_product_qty(self, product, location, qty_type):
        ...
        if qty_type == 'on_hand':
            ...
            if location:
                # virtual_available is computed globally, so we use
                # on_hand at location + (incoming - outgoing) as approximation
                quants = self.env['stock.quant'].search([
                    ('product_id', '=', product.id),
                    ('location_id', '=', location.id),
                ])
                ...
                # Get incoming/outgoing moves for this location
    ...
    def get_all_product_stock(self, location_id=False):
        ...
        if qty_type == 'on_hand':
            # Total physical stock (quantity field on quant)
            for q in quants:
                pid = q.product_id.id
            ...
            # On hand - reserved + incoming - outgoing
            for q in quants:
                pid = q.product_id.id
                if pid not in result:
                    result[pid] = 0
