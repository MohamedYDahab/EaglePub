...
class PosSession(models.Model):
    ...
    def _loader_params_product_product(self):
        ...
        result['search_params']['fields'].append('allow_negative_stock')
        return result
    ...
    def _neg_stock_qty_map(self, products, location, qty_type):
        """Quantity per product id, defaulting to 0 when no quant exists.
        ...
        for group in groups:
            on_hand = group['quantity'] or 0.0
            reserved = group['reserved_quantity'] or 0.0
            result[group['product_id'][0]] = (
                on_hand if qty_type == 'on_hand' else on_hand - reserved
            )
    ...
    def get_all_product_stock(self, location_id=False, config_id=False,
                              product_ids=None):
        """Stock per product for the POS badges."""
        if not self._neg_stock_bool('enabled') or \
        ...
        return self._neg_stock_qty_map(
            products.filtered(lambda p: p.type == 'product'),
