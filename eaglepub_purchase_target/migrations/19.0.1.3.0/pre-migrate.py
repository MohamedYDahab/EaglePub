...
def migrate(cr, version):
    ...
    cr.execute("""
        ...
        WHERE table_name = 'purchase_target_line' AND column_name = 'target_type'
    """)
    if not cr.fetchone():
        return

    cr.execute("DELETE FROM purchase_target_line WHERE target_type = 'amount'")
    _logger.warning(
        "eaglepub_purchase_target: removed %s amount-based target line(s); "
